...
- **Final report**: downloadable as Markdown.
...
- **RAG extension**: replace `ResearchAgent._gather_context` with a retriever over Chroma/FAISS over user-uploaded PDFs.
- **Persistent memory**: add a SQLite checkpointer so workflows can resume after a crash and accumulate cross-run knowledge.
- **Benchmark harness**: build a corpus of `(query, gold_report)` pairs and report aggregate score distributions for baseline vs multi-agent.
- **Streaming UI**: switch from blocking invokes to LangGraph's astream so the UI updates token-by-token.
- **Cost tracking**: instrument the LLM factory to record token usage per agent and surface $ per run.
